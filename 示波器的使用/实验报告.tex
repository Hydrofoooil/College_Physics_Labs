...
\section{预习报告}
    ...
    首先调节TIME/DIV扫描时基信号，仔细调节信号发生器，使示波器全屏显示1个、2个$\dots$等完整的波形，相应地从信号发生器上读出各种情况下的$f_y$信号频率，根据$f_y=nf_x$求出相应的$f_x$；
...
\section{结果与分析}
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=.3\textwidth]{figures/二极管测量.jpg}
        \caption{测量时示波器显示图形}
        \label{fig:图4}
    \end{figure}
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=.3\textwidth]{figures/相位差测量.jpg}
        \caption{测量时示波器显示图形}
        \label{fig:图5}
    \end{figure}
